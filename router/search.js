...
async function firstDelete() {
  try {
    const lastRows = await First.findAll({
      order: [['createdAt', 'DESC']], // createdAt을 기준으로 내림차순 정렬하여 최신 데이터를 가져옵니다.
    });
  
    if (lastRows.length >= 2) {
      // 조회된 결과가 2개 이상인 경우
      console.log('테이블의 가장 마지막 행이 2개 이상입니다.');
      // 최초의 데이터를 제외한 나머지 데이터를 삭제합니다.
...
router.get("/attraction", async (req, res) => {
  let typeIndex = "tourist_attraction";
  console.log("확인1");
  ...
  //   const fields = "fields=formatted_address%2Cname%2Cgeometry";
...
router.get("/temp", async (req, res) => {
  ...
  let pickPlan = await Plan.findOne({
    where: {
      id: urlid, // URL에서 받은 id 변수를 사용하여 쿼리 설정
    },
  });
  ...
  if (pickPlan) {
    ...
    // Handle the case where the plan with the provided id is not found
